...
void lcd_init(void)
{
  ...
  lcd_write_command(0x28); //4-разрядная шина, 2 - строки
#else
  lcd_write_command(0x38); //8-разрядная шина, 2 - строки
#endif
  lcd_write_command(0x08);
  lcd_write_command(0x0c);  //дисплей вкл, курсор и мерцание выключены
  lcd_write_command(0x01);  //0b00000001 - очистка дисплея
  _delay_ms(2);
  lcd_write_command(0x06);  //0b00000110 - курсор движется вправо, сдвига нет
}
...
void display_ad_custom_symbol(char *bytes, char number){
	lcd_write_command(0x40 + number*8);
	for (int i = 0; i < 8; i++){
		lcd_write_data(bytes[i]);
	}
